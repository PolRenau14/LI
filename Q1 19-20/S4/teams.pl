...
incompatibleWorkers(W1,W2):- worker(W1), worker(W2), W1 \= W2,workerScore(W1,S1), workerScore(W2,S2), S is S1+S2, maxScore(Max), S > Max.
complexWorkers(W1,W2):- worker(W1), worker(W2), W1 \= W2,workerScore(W1,S1), workerScore(W2,S2), S is S1+S2, complexThreshold(Max), S >= Max.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
writeClauses(MaxComplexTeams):-
    ...
    eachWorkerHaveTeam,
    thereIsNoMaxScore,
  %  thereExactNumTeams, es redundant ja que com fem que tots els equips estiguin sized, i que tots els W tinguin team. ja estaria.
    computeComplex,
    noMaxComplex(MaxComplexTeams),
    true,!.
writeClauses(_):- told, nl, write('writeClauses failed!'), nl,nl, halt.

eachTeamIsSized:- team(T),findall(wt(W,T),worker(W),L), minSize(MinS), atLeast(MinS,L), maxSize(MaxS), atMost(MaxS,L),fail.
eachTeamIsSized.

eachWorkerHaveTeam:- worker(W), findall(wt(W,T),team(T),L), exactly(1,L),fail.
eachWorkerHaveTeam.

thereIsNoMaxScore:- incompatibleWorkers(W1,W2), team(T), writeClause([-wt(W1,T),-wt(W2,T)]),fail.
thereIsNoMaxScore.

thereExactNumTeams:- findall(t(T),team(T),L), numTeams(NT), exactly(NT,L),fail.
thereExactNumTeams.

computeComplex:- team(T),complexWorkers(W1,W2),writeClause([-wt(W1,T), -wt(W2,T), ct(T)]) ,fail.
computeComplex.

noMaxComplex(MaxC):- findall(ct(T),team(T),L), atMost(MaxC,L),fail.
noMaxComplex(_).
...
costOfThisSolution(M,Cost):- findall(C,member(ct(C),M),L), sort(L,L1), length(L1,Cost), !.

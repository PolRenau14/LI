...
heli( [], AccumulatedDistance, [CurrentPoint|RouteSoFar], [] ):-
  helicopterBasePoint(Base), distanceBetweenTwoPoints(Base,CurrentPoint,D), NewDist is AccumulatedDistance + D,
  storeRouteIfBetter(NewDist,[Base,CurrentPoint|RouteSoFar]),fail.
...
heli( PassengersToPickUp, AccumulatedDistance, [CurrentPoint|RouteSoFar], DestinationsOfPassengersInHelicopter ):-
  length(DestinationsOfPassengersInHelicopter,N),N < 2,pert_con_resto([Porigen,PDesti],PassengersToPickUp,Rest),
  distanceBetweenTwoPoints(Porigen,CurrentPoint,D), NewDist is AccumulatedDistance + D,
  heli(Rest,NewDist,[Porigen,CurrentPoint|RouteSoFar],[PDesti|DestinationsOfPassengersInHelicopter]).


% Drop off one of the passengers in the helicopter:
heli( PassengersToPickUp, AccumulatedDistance, [CurrentPoint|RouteSoFar], DestinationsOfPassengersInHelicopter ):-
  pert_con_resto(Point,DestinationsOfPassengersInHelicopter,Resto), distanceBetweenTwoPoints(Point,CurrentPoint,D),
  NewDist is D + AccumulatedDistance, heli(PassengersToPickUp,NewDist,[Point,CurrentPoint|RouteSoFar],Resto).

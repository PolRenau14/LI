...
maxDistance(C1,C2,C3):-     trip(C1-C2),trip(C2-C3),dist(C1,C2,D1),dist(C2,C3,D2),D is D1+D2, maxDist(MD),D > MD.
...
everyCityHaveBus:- city(C),day(D),findall(bdc(B,D,C),bus(B),L),exactly(1,L),fail.
everyCityHaveBus.

everyBusHaveCity:- bus(B), day(D), findall(bdc(B,D,C),city(C),L),exactly(1,L),fail.
everyBusHaveCity.

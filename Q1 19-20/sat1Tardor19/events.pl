...
writeClauses:-
    everyEventOneDay,
    everyEventOneModerator,
    computeMaxEventsPerDay,
    computeMaxDaysPerModerator,
    eventsModerator,
    eventsDay,
    uneDaysEventsModerator,
    true,!.                    % this way you can comment out ANY previous line of writeClauses
writeClauses:- told, nl, write('writeClauses failed!'), nl,nl, halt.

everyEventOneDay:- event(E),findall(ed(E,D),day(D),L),exactly(1,L),fail.
everyEventOneDay.

everyEventOneModerator:- event(E),findall(em(E,M),moderator(M),L),exactly(1,L),fail.
everyEventOneModerator.

computeMaxEventsPerDay:- day(D),findall(ed(E,D),event(E),L),maxEventsPerDay(M),atMost(M,L),fail.
computeMaxEventsPerDay.

computeMaxDaysPerModerator:- moderator(M),findall(md(M,D),day(D),L),maxDaysPerModerator(Max),atMost(Max,L),fail.
computeMaxDaysPerModerator.

eventsModerator:- event(E),eventModerators(E,Moderators),moderator(M),\+member(M,Moderators),writeClause([-em(E,M)]),fail.
eventsModerator.

eventsDay:- event(E),eventDays(E,Days),day(D),\+member(D,Days),writeClause([-ed(E,D)]),fail.
eventsDay.

uneDaysEventsModerator:- event(E),day(D),moderator(M),writeClause([-ed(E,D),-em(E,M),md(M,D)]),fail.
uneDaysEventsModerator.

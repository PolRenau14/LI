...
incompatibleWorkers(W1,W2):- worker(W1),worker(W2),W1 \= W2, maxScore(M),score(W1,S1),score(W2,S2),N is S1 +S2, N > M.
...
satVariable(t(T)):- team(T). %aquesta variable serveix per indicar el nombre de Teams.
...
everyWorkerOneTeam:- worker(W),findall(wt(W,T),team(T),L),exactly(1,L),fail.
everyWorkerOneTeam.

everyTeamIsSized:- team(T), findall(wt(W,T),worker(W),L), maxSize(N1), atMost(N1,L),minSize(N2), atLeast(N2,L),fail.
everyTeamIsSized.

computeIncompatible:- team(T),incompatibleWorkers(W1,W2),writeClause([-wt(W1,T),-wt(W2,T)]),fail.
computeIncompatible.

exactNumberOfTeams:- findall(t(T),team(T),L),numTeams(NT),exactly(NT,L).
exactNumberOfTeams.

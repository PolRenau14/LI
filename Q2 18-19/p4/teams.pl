...
writeClauses(MaxComplexTeams):-
    ...
    sizeMinTeam,
    complexTeam,
    lessThanXComplexTeams(MaxComplexTeams),
    true,!.
writeClauses(_):- told, nl, write('writeClauses failed!'), nl,nl, halt.

everyWorkerOneTeam:- worker(W), findall(wt(W,T), team(T), Lits), exactly(1,Lits), fail.
everyWorkerOneTeam.

maximumScore:- incompatibleWorkers(W1,W2), team(T), writeClause([-wt(W1,T),-wt(W2,T)]), fail.
maximumScore.

sizeMaxTeam:- maxSize(M), team(T), findall(wt(W,T), worker(W), Lits), atMost(M,Lits), fail.
sizeMaxTeam.

sizeMinTeam:- minSize(M), team(T), findall(wt(W,T), worker(W), Lits), atLeast(M,Lits), fail.
sizeMinTeam.


complexTeam:- team(T), complexWorkers(W1,W2), writeClause([-wt(W1,T),-wt(W2,T),t(T)]), fail.
complexTeam.

lessThanXComplexTeams(X):- findall(t(T),team(T),Lits), atMost(X,Lits), fail.
lessThanXComplexTeams(_).

...
matrixByRows(L,ColSums,[L]):- length(L,ColSums).
matrixByRows(L,ColSums,[Ini|Matrix]):-
    append(Ini,Fin,L),
    length(Ini,ColSums),
    matrixByRows(Fin,ColSums,Matrix).

expresionSuma([V],V).
expresionSuma([V|Vars], V+E):- expresionSuma(Vars,E).
    
declareConstraints([M],[S]):- expresionSuma(M,Res), Res #= S.
declareConstraints([M|Matrix],[S|Sums]):-
    expresionSuma(M,Res),
    Res #= S,
